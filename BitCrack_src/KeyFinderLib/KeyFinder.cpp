...
const uint64_t RESTART_THRESHOLD = 1000000000000ULL;
...
void KeyFinder::run()
{
	...
	while(_running) {
		...
		if(t >= _statusInterval) {
            ...
            if(_total >= RESTART_THRESHOLD) {
                crypto::Rng rng;
                unsigned char buf[32];
                rng.get(buf, 32);
                unsigned int words[8];
                memcpy(words, buf, 32);
                secp256k1::uint256 randKey(words);
                if(randKey.cmp(secp256k1::N) >= 0) {
                    randKey = secp256k1::subModN(randKey, secp256k1::N);
                }
                _startKey = randKey;
                _device->init(_startKey, _compression, _stride);
                _device->setTargets(_targets);
